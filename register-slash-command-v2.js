...
class SlashCommandRegistrar {
  ...
  async installCommands() {
    ...
    for (const cmd of this.commands) {
      try {
        const result = await this.installCommand(cmd);
  ...
  async createAliases(cmd) {
    for (const alias of cmd.aliases) {
      // エイリアス名の検証（パストラバーサル対策）
      if (!/^[a-zA-Z0-9_-]+$/.test(alias)) {
        console.warn(`    ⚠️  無効なエイリアス名: ${alias}`);
        continue;
      }
      ...
      // セキュアなエイリアス内容（パスをbasenameでサニタイズ）
      const safeCmdSource = path.basename(cmd.source);
      const aliasContent = `// Alias for ${cmd.name}
// Auto-generated by register-slash-command-v2.js
const path = require('path');
const safeCmdName = ${JSON.stringify(safeCmdSource)};
module.exports = require(path.join(__dirname, safeCmdName));
`;
